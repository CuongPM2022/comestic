...
public class CartDAO extends AbstractDAO<CartModel> implements ICartDAO {
	...
	private List<CartItemModel> listCartItemModelError;
	...
	public Long save(CartModel cartModel) {
		...
		this.cartId = insert(sql.toString(), this.listInput);
		
		this.listCartItemModelError = new ArrayList<CartItemModel>();
		for(CartItemModel cartItem:cartModel.getListCartItem()) {
			this.cartItemModel = cartItemDAO.findOneByVarietyId(cartItem.getVarietyId());
			this.cartItemModel.setUserQuantity(cartItem.getUserQuantity());
			if(this.cartItemModel.getUserQuantity() <= this.cartItemModel.getNumber()) {
				this.cartItemModel.setCartId(this.cartId);
				this.cartItemDAO.save(this.cartItemModel);
				this.totalMoney += this.cartItemModel.getUserQuantity() * this.cartItemModel.getPrice();
				this.importMoney += this.cartItemModel.getUserQuantity() * this.cartItemModel.getPrice() 
									* (1- (this.cartItemModel.getPercentDes() + 0.0)/100);
				
			}
			else {
				this.listCartItemModelError.add(this.cartItemModel);
			}
		}
